...
key-store 생성 - 앱에서 keystore 참조(key property파일이 git등의 오픈소스로 공개되지않게 git.ignore에 명시)     
- 터미널로 프로젝트 폴더에서 앱번들 빌드(build/app/output/bundle/release내에 생성) - play console에 엡 정보와 함께 번들 등록      
